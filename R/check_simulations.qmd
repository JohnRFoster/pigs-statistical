...
file_dir <- "../analysis/simulation/modifiedDM_betaSurvival_dataByMethod"
...
method_lookup_p <- tibble(
  idx = 1:3,
  method = c("Firearms", "Snare", "Traps")
)

df <- read_csv(file.path(file_dir, "summaryResidual_uniqueArea.csv")) |> 
  left_join(method_lookup_p)
...
df |> 
  ggplot() +
  aes(y = med_abundance, x = abundance, color = simulation) +
  geom_point(size = 0.5) +
